...
const CONFOUNDER_RULES: Rule[] = [
  { keywords: ["caffeine", "coffee", "tea"], type: "behavioral", control: "Hold your caffeine intake (coffee, tea, energy drinks) constant throughout the experiment." },
  ...
  { keywords: ["illness", "sick", "flu"], type: "physiological", control: "Note any illness; pause judgement on those days as it distorts the signal." },
...
export function detectConfounders(names: string[]): Confounder[] {
  return names.map((name) => {
    ...
    const rule = CONFOUNDER_RULES.find((r) => r.keywords.some((k) => lower.includes(k)));
